...
function tmux-browser --wraps="tmux" --description="tmux browser"
    # `command` is needed in case `lfcd` is aliased to `lf`.
    # Quotes will cause `cd` to not change directory if `lf` prints nothing to stdout due to an error.
    cd (eval $HOME/.config/tmux/scripts/tmux-browser $argv)
end

# function lfcd
#     # Use command to ensure we're calling the correct `lfpp`
# end
